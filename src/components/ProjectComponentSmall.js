...
function ProjectComponentSmall(props) {
  ...
  if (props.links) {
    footnotes = props.links.map((row, index) => {
      return (
        <li key={index}>
          <a
            key={index}
  ...
  return (
    <>
      <div
        className={
          "p-6 text-black w-10/12 shadow-xl sm:w-9/12 mt-5 cursor-pointer border-purple-400 relative rounded-md drop-shadow-md border-2"
        }
        ...
        <div className="divide-y-2">
          ...
          <p className="pt-2 " style={{whiteSpace: 'pre-wrap'}}>
            {props.text.substring(0, 178) +
              (clicked
                ? props.text.substring(178, props.text.length)
                : ".. [View More]")}
          </p>

          {clicked && footnotes && <ul className="mt-1 pt-1"> {footnotes}</ul>}
        </div>
      </div>
    </>
  );
